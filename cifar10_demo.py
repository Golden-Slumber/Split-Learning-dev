...
class WirelessSplitNetV2(nn.Module):
    def __init__(self, next_layer_neurons, pre_layer_neurons, device=None):
        super(WirelessSplitNetV2, self).__init__()
        self.device = device
...
def cifar10_training(args, n_devices, next_layer_neurons, pre_layer_neurons):
    use_cuda = not args.no_cuda and torch.cuda.is_available()
    torch.manual_seed(args.seed)
    device = torch.device("cuda" if use_cuda else "cpu")
    ...
    if args.save_model:
        torch.save(model.state_dict(), 'multi_modality_cifar10_n_devices_' + str(n_devices) + '.pt')
...
if __name__ == '__main__':
    ...
    # cifar10_training(args, n_devices, next_layer_neurons, pre_layer_neurons)

    # load model
    model_state_dict = torch.load('multi_modality_cifar10_n_devices_' + str(n_devices) + '.pt')
    model = WirelessSplitNetV2(next_layer_neurons, pre_layer_neurons, device=device).to(device)
    ...
    eta_list = [6e-5, 6e-5, 6e-5, 5e-5, 5e-5, 5e-5, 4e-5, 4e-5]
    # eta_list = [5e-4, 5e-4, 4e-4, 4e-4, 3e-4, 3e-4, 2e-4, 2e-4]
    ...
    for r in range(repeat):
        ...
        out_file_name = home_dir + 'Outputs/aircomp_based_split_inference_demo_' + data_name + '_repeat_' + str(
            r+20) + '_partial_results_v3.npz'
        numpy.savez(out_file_name, res=stored_results, obj=stored_objectives)
    out_file_name = home_dir + 'Outputs/aircomp_based_split_inference_demo_' + data_name + '_repeats_' + str(
        repeat) + '_total_results_v3.npz'
